...
class Pacientes extends BaseController
{
    ...
    public function index($activo = 1){
        ...
        $data = ['titulo'=> 'Pacientes', 'datos'=> $pacientes];
    ...
    public function insertar(){
        ...
        if($this->pacientes->save(['nombrePaciente'=>$this->request->getPost('nombre'),
        ...
        'generoPaciente'=>$this->request->getPost('genero')
            ...
            return redirect()->to(base_url().'/pacientes')->with('mensaje', 'Se ha ingresado correctamente.');
        }else{
            return redirect()->to(base_url().'/pacientes')->with('mensajeError', 'No se ha ingresado correctamente.');
        }
    }else{
    ...
    public function actualizar(){
        if($this->request->getMethod()=="post" && $this->validate($this->reglas)){
        if($this->pacientes->update($this->request->getPost('id'), 
        ...
        'generoPaciente'=>$this->request->getPost('genero')
            ...
            return redirect()->to(base_url().'/pacientes')->with('mensaje', 'Se ha modificado correctamente.');
        }else{
            return redirect()->to(base_url().'/pacientes')->with('mensajeError', 'No se ha modificado correctamente.');
        }
     }else{
    ...
    public function generaPacientesPDF($fechaInicio, $fechaFin)
    {
        ...
        /*$anamnesis = $this->pacientes->listarFiltrosAnamnesisRealizadas($idPaciente, $fechaInicio, $fechaFin);
        $diagnosticos = $this->pacientes->listarFiltrosDiagnosticosRealizadas($idPaciente, $fechaInicio, $fechaFin);
        $tratamientos = $this->pacientes->listarFiltrosTratamientosRealizadas($idPaciente, $fechaInicio, $fechaFin);*/
        $procedimientos = $this->pacientes->listarFiltrosProcedimientossRealizadas($fechaInicio, $fechaFin);
//        $cantidadProcedimientos = $this->pacientes->listarFiltrosCantidadProcedimientossRealizadas($idPaciente, $fechaInicio, $fechaFin);
